...
REMOTE_ROOT="/root/work/mwg-ew-patent"
SHELL_WRAPPER="$ROOT_DIR/scripts/asi2_shell.sh"
...
usage() {
  cat >&2 <<'EOF'
Usage:
  scripts/ASI2_job.sh start <job-name> <log-path> "<remote command>"
  scripts/ASI2_job.sh status <job-id> [tail-lines]
  scripts/ASI2_job.sh logs <job-id> [tail-lines]
