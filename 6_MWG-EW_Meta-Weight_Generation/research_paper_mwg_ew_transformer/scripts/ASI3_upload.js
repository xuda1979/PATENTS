...
process.env.HUANXIN_PROFILE_COPY_NAME = process.env.HUANXIN_PROFILE_COPY_NAME || 'mwg-ew-ASI3-upload';
...
if (process.env.ASI3_ALLOW_BROWSER !== '1') {
  console.error(
    'Refusing to run ASI3_upload.js because it opens Huanxin browser automation. ' +
      'Use S3 scripts, an existing daemon shell, or set ASI3_ALLOW_BROWSER=1 only when browser control is allowed.'
  );
  process.exit(2);
}
...
function parseArgs(argv) {
  const args = {
    ...
    chunkChars: parseInt(process.env.ASI3_UPLOAD_CHUNK_CHARS || '50000', 10),
  };
  for (let i = 0; i < argv.length; i += 1) {
    ...
    if (token === '--remote-dir') {
      ...
      args.chunkChars = parseInt(argv[++i], 10);
    } else {
      throw new Error(`Unknown argument: ${token}`);
    }
  }
  if (!Number.isFinite(args.chunkChars) || args.chunkChars < 1024) {
    throw new Error(`Invalid --chunk-chars: ${args.chunkChars}`);
  }
  return args;
}
...
async function main() {
  ...
  const chunks = chunkString(archiveBase64, args.chunkChars);
  const remoteBase = `/tmp/mwg-ew-ASI3-upload-${Date.now()}`;
