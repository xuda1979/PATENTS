...
REPEATS="${ASI2_RELIABILITY_REPEATS:-3}"
DEVICE_SETS="${ASI2_RELIABILITY_DEVICE_SETS:-0 0,1 0,1,2,3}"
RANKS="${ASI2_RELIABILITY_RANKS:-64,128,256}"

mkdir -p results logs
TS="$(date -u +%Y%m%dT%H%M%SZ)"
RUN_ID="ASI2_reliability_scaling_${TS}"
LOG="logs/${RUN_ID}.log"
...
{
  echo "ASI2 MWG-EW reliability scaling"
  echo "run_id=$RUN_ID"
...
PY
  ...
  python3 experiments/aggregate_ASI2_scaling.py \
    --glob "${OUTDIR}/ASI2_mwg_transformer_*.json" \
