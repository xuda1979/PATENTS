...
function usage() {
  console.error('Usage: node scripts/ASI3_daemon_fetch_files.js --file <remote-path> <local-path> [--file ...]');
  process.exit(2);
}
...
function extractPayload(output) {
  const begin = '__ASI3_DAEMON_FETCH_BEGIN__';
  const end = '__ASI3_DAEMON_FETCH_END__';
...
const hardTimeoutMs = parseInt(process.env.HUANXIN_DAEMON_HARD_TIMEOUT_MS || '30000', 10);
const fetched = [];
...
function runRemoteJson(remoteBody) {
  const wrappedCode = [
    ...
    'print("__ASI3_DAEMON_FETCH_BEGIN__")',
    'print(payload)',
    'print("__ASI3_DAEMON_FETCH_END__")',
  ].join('\n');
