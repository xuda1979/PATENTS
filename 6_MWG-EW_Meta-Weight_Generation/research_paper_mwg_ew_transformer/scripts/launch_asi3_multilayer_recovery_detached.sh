...
LAYERS="${LAYERS:-16,17,18}"
cd "$PROJECT_ROOT"

mkdir -p logs results
TS="$(date -u +%Y%m%dT%H%M%SZ)"
RUN_ID="asi3_multilayer_recovery_${TS}"
...
PY
  ...
  echo "ASI3 multi-layer recovery"
  echo "run_id=$RUN_ID"
  ...
  IFS="," read -r -a layer_array <<< "$LAYERS_STR"
  for LAYER in "${layer_array[@]}"; do
    LAYER="${LAYER//[[:space:]]/}"
    [[ -n "$LAYER" ]] || continue
    write_status running "train_layer${LAYER}_r384"
    echo "=== training layer ${LAYER} ==="
    date -u
    export LAYER
    export OUTDIR="$RUN_OUTDIR/layer${LAYER}"
    mkdir -p "$OUTDIR/checkpoints"
    bash scripts/run_asi1_quality_distillation.sh

    BASE_CKPT="$RUN_OUTDIR/layer${LAYER}/checkpoints/mwg_expert_residual_r384.pt"
    LMCE_CKPT="$RUN_OUTDIR/layer${LAYER}/checkpoints/mwg_expert_residual_r384_belle_lmce_cal_1200.pt"
    if [[ ! -f "$BASE_CKPT" ]]; then
      write_status failed "missing base checkpoint for layer ${LAYER}: $BASE_CKPT"
      echo "ERROR: missing $BASE_CKPT" >&2
      exit 2
    fi
    LMCE_TEXT_ARGS=(--texts "$LMCE_TEXTS")
    write_status running "lmce_calibrate_layer${LAYER}_1200"
    python3 experiments/mwg_logit_calibrate.py \
      --model-dir "$MODEL_DIR" \
      --layer "$LAYER" \
      --checkpoint "$BASE_CKPT" \
      --out-checkpoint "$LMCE_CKPT" \
      --out-json "$RUN_OUTDIR/ppl_layer${LAYER}_r384_belle_lmce_cal_1200_heldout256_eval128_tb2.json" \
      --objective lm_ce \
  ...
  write_status running multi_layer_ppl_eval
  HYBRID_TEXT_ARGS=(--texts "$LMCE_TEXTS")
  PATCH_ARGS=()
  for LAYER in "${layer_array[@]}"; do
    LAYER="${LAYER//[[:space:]]/}"
    [[ -n "$LAYER" ]] || continue
    LMCE_CKPT="$RUN_OUTDIR/layer${LAYER}/checkpoints/mwg_expert_residual_r384_belle_lmce_cal_1200.pt"
    if [[ -f "$LMCE_CKPT" ]]; then
      PATCH_ARGS+=(--patch "${LAYER}:${LMCE_CKPT}")
    fi
  done
  echo "=== multi-layer PPL eval with patches: ${PATCH_ARGS[*]} ==="
  date -u
  python3 experiments/mwg_ppl_patch_eval.py \
    --model-dir "$MODEL_DIR" \
    --seq "${LMCE_SEQ:-256}" \
    --text-batch "${LMCE_TEXT_BATCH:-2}" \
    --eval-batches "${LMCE_EVAL_BATCHES:-128}" \
    --dtype "${LMCE_DTYPE:-fp32}" \
    --require-texts \
    --out-json "$RUN_OUTDIR/multi_layer_ppl_eval.json" \
    "${PATCH_ARGS[@]}" \
    "${HYBRID_TEXT_ARGS[@]}"
...
echo "ASI3 multi-layer recovery detached"
echo "pid=$PID"
echo "project_root=$PROJECT_ROOT"
echo "run_id=$RUN_ID"
echo "layers=$LAYERS"
