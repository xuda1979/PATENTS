...
function usage() {
  console.error('Usage: node scripts/ai3_daemon_upload_files.js --file <local-path> <remote-path> [--file ...]');
  process.exit(2);
}
...
function extractPayload(output) {
  const begin = '__AI3_DAEMON_UPLOAD_BEGIN__';
  const end = '__AI3_DAEMON_UPLOAD_END__';
...
const chunkBytes = parseInt(process.env.AI3_DAEMON_UPLOAD_CHUNK_BYTES || '700', 10);
const uploaded = [];
...
function runRemoteJson(remoteBody) {
  const wrappedCode = [
    ...
    'print("__AI3_DAEMON_UPLOAD_BEGIN__")',
    'print(payload)',
    'print("__AI3_DAEMON_UPLOAD_END__")',
  ].join('\n');
  const remoteCommand = `python3 - <<'PY'\nimport base64, hashlib, json, os, pathlib\n${wrappedCode}\nPY`;
  const attempts = parseInt(process.env.AI3_DAEMON_UPLOAD_RETRIES || '4', 10);
  ...
  throw new Error('ai3_shell failed after stale daemon retries');
}
...
for (const pair of pairs) {
  ...
  const tmpRemote = `${remote}.ai3-upload-${Date.now()}-${Math.random().toString(16).slice(2)}.tmp`;
