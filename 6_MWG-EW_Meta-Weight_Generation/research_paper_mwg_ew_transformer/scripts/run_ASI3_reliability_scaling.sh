...
REPEATS="${ASI3_RELIABILITY_REPEATS:-3}"
DEVICE_SETS="${ASI3_RELIABILITY_DEVICE_SETS:-0 0,1 0,1,2,3}"
RANKS="${ASI3_RELIABILITY_RANKS:-64,128,256}"

mkdir -p results logs
TS="$(date -u +%Y%m%dT%H%M%SZ)"
RUN_ID="ASI3_reliability_scaling_${TS}"
LOG="logs/${RUN_ID}.log"
...
{
  echo "ASI3 MWG-EW reliability scaling"
  echo "run_id=$RUN_ID"
...
PY
  ...
  python3 experiments/aggregate_ASI3_scaling.py \
    --glob "${OUTDIR}/ASI3_mwg_transformer_*.json" \
