"""Aggregate repeated ASI3 MWG-EW benchmark runs.

This script consumes the per-run JSON files written by
``mwg_transformer_ASI3_benchmark.py`` and creates a compact reliability/scaling
summary for paper tables. It intentionally uses only the Python standard
library so it can run in constrained Huanxin environments.
"""
