...
Run locally:
    python3 experiments/mwg_transformer_ai3_benchmark.py --preset tiny

Run a fast remote smoke test on AI3:
    python3 experiments/mwg_transformer_ai3_benchmark.py --preset ai3_smoke

Run the full AI3 setting:
    ASCEND_RT_VISIBLE_DEVICES=0,1,2,3 python3 -m torch.distributed.run \
      --nproc_per_node=4 experiments/mwg_transformer_ai3_benchmark.py \
      --preset ai3_large
"""
...
def write_outputs(payload: dict[str, Any], outdir: Path) -> tuple[Path, Path]:
    ...
    json_path = outdir / f"ai3_mwg_transformer_{tag}.json"
    md_path = outdir / f"ai3_mwg_transformer_{tag}.md"
