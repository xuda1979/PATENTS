...
## 13. Real Standard Falcon Benchmark Results / 真实标准 Falcon 基准实验结果

> **All data below comes from actual algorithm execution on 2026-03-09. No values are simulated.**

### 13.1 Experiment Setup / 实验设置
...
### 13.2 Key and Signature Sizes / 密钥与签名大小

| Parameter | Security | SK (bytes) | VK (bytes) | Signature (bytes) |
|-----------|----------|------------|------------|-------------------|
...
### 13.3 Performance Results / 性能结果

#### Key Generation (ms)

| Parameter | Mean | Median | Stdev | Min | Max |
|-----------|------|--------|-------|-----|-----|
| Falcon-256 | 1,204.9 | 1,066.0 | 692.7 | 592.1 | 1,956.6 |
| Falcon-512 | 5,749.3 | 5,410.5 | 1,869.1 | 4,072.8 | 7,764.6 |
| Falcon-1024 | 21,201.6 | 20,722.9 | 1,362.7 | 20,142.8 | 22,739.1 |

#### Signing (ms)

| Parameter | Mean | Median | Stdev | Min | Max |
|-----------|------|--------|-------|-----|-----|
| Falcon-256 | 16.8 | 17.1 | 0.9 | 15.6 | 18.6 |
| Falcon-512 | 41.6 | 40.2 | 5.2 | 36.6 | 54.0 |
| Falcon-1024 | 91.7 | 85.8 | 18.4 | 74.0 | 119.1 |

#### Verification (ms)

| Parameter | Mean | Median | Stdev | Min | Max |
|-----------|------|--------|-------|-----|-----|
| Falcon-256 | 3.8 | 3.7 | 0.4 | 3.3 | 4.4 |
| Falcon-512 | 9.7 | 9.6 | 0.9 | 8.4 | 11.2 |
| Falcon-1024 | 20.8 | 19.5 | 2.8 | 18.2 | 26.1 |

### 13.4 Relevance to QTS-Falcon Threshold Scheme / 与门限方案的关联

- **Verification transparency**: threshold Falcon verification is identical to standard Falcon verification (~10 ms for Falcon-512).
- **Signature size preservation**: the threshold scheme produces standard Falcon signatures (666 bytes for Falcon-512) with no additional metadata.
- **Signing overhead baseline**: single-party signing at ~42 ms for Falcon-512 provides the comparison floor for the threshold MPC-based signing protocol.
- **Key generation**: NTRU key generation (one-time setup) at ~5.7 s for Falcon-512 is acceptable; in the threshold DKG, this cost is distributed and amortized.

### 13.5 Raw Data / 原始数据

Full per-iteration raw data and machine-readable summary:
- `real_benchmarks/falcon_standard/results/raw_benchmark.csv` — 69 individual measurements
- `real_benchmarks/falcon_standard/results/summary.json` — aggregate statistics
- `real_benchmarks/falcon_standard/results/report.md` — detailed analysis report

### 13.6 Previous Simulation Notice / 之前的仿真说明

The repository previously contained simulation-based results under `simulation/`. Those files are **deprecated and non-citable** — they were not generated by a real Falcon implementation and do not constitute valid experimental evidence. See `simulation/README.md` for details.
