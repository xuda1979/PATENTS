...
**Claim 7.** The system or method according to claim 1 or 2, further comprising a dynamic node management module supporting:
...
**Claim 8.** The system or method according to claim 7, wherein the dynamic node management module employs proactive secret sharing protocol with the property:
...
**Claim 10.** The system or method according to claim 1 or 2, employing a $(t, n)$ threshold structure wherein:
...
- the signed instructions authorize atomic asset transfers between different blockchain networks with finality guarantees.
...
- wherein on-chain verification gas cost is reduced by approximately 72% compared to threshold Dilithium signatures.
...
**Claim 31.** The system according to claim 1, wherein the system achieves the following performance characteristics:
- online signing latency: $\leq 15$ milliseconds for $(5,7)$ threshold with Falcon-512;
...
- on-chain verification gas cost: approximately 50,000 gas on Ethereum (72% reduction vs. Dilithium);
- throughput: $\geq 67$ signatures per second per node cluster;
- rejection sampling efficiency: 65.2% acceptance rate, 1.53 expected attempts.
...
**Claim 34.** The method according to claim 2, wherein the distributed coin flip in step S5 achieves bias-resistance through:
- each party $P_i$ broadcasting commitment $D_i = H(\rho_i)$ for random $\rho_i \leftarrow \{0,1\}^{256}$;
- after receiving all commitments, each party revealing $\rho_i$;
- verifying $H(\rho_i) = D_i$ for all $i$;
- computing combined randomness $\rho = H(\rho_1 \| \rho_2 \| \cdots \| \rho_n)$;
...
**Claim 35.** The system according to claim 1, further comprising a batch signing optimization wherein:
- multiple messages $M_1, \ldots, M_k$ are signed in parallel within single protocol execution;
- Beaver triples are shared across batch: $\binom{n}{2} \cdot k$ triples for $k$ signatures;
- per-signature amortized communication reduced by factor $k$;
- latency for $k$ signatures approximately $(1 + 0.1k)$ times single signature latency.
...
├── Claim 3 (NTT Linearity / CRT Isomorphism)
├── Claim 4 (Variance-Preserving Aggregation)
├── Claim 5 (Commit-Reveal Protocol)
├── Claim 6 (Beaver-Based Norm Computation)
├── Claim 9 (Hardware Acceleration)
├── Claim 10 (Threshold Structure Properties)
├── Claim 11 (Standard Format Compatibility)
├── Claim 12 (MPC-XGCD for Trapdoor)
├── Claim 17 (Cross-Chain Application)
│   └── Claim 18 (Bridge Architecture)
├── Claim 20 (Privacy Mechanisms)
├── Claim 22 (PQ-Secure OT Extension)
├── Claim 24 (Malicious Party Detection)
├── Claim 25 (Fault Tolerance)
├── Claim 26 (XGCD Algorithm Details)
├── Claim 27 (Side-Channel Protection)
├── Claim 28 (TEE Integration)
├── Claim 31 (Performance Characteristics)
├── Claim 32 (Security Proofs)
└── Claim 35 (Batch Signing)

Claim 2 (Method - Independent)
├── Claim 13 (6-Round Communication)
├── Claim 14 (Local Share Computation)
├── Claim 15 (Aggregation and Compression)
├── Claim 16 (Standard Verification)
├── Claim 19 (Quantum Security Basis)
├── Claim 21 (Parameter Scaling Bounds)
├── Claim 23 (ZKP for Gaussian Samples)
├── Claim 29 (Dual-Modulus Architecture)
├── Claim 30 (Early Rejection Optimization)
└── Claim 34 (Bias-Resistant Coin Flip)

Claims 1 or 2 (System or Method)
├── Claim 7 (Dynamic Node Management)
│   └── Claim 8 (Proactive Secret Sharing)
└── Claim 33 (Computer-Readable Medium)
```
...
The independent claims define:
1. A system comprising distributed key generation, arithmetic-shared transform-domain computation, collaborative rejection sampling, and signature aggregation modules (Claim 1)
2. A method comprising distributed key generation, message preprocessing, local sampling, collaborative rejection sampling, and signature aggregation steps (Claim 2)

Key innovations protected:
- Zero-communication distributed transform-domain computation using linearity property
- Scaled Gaussian parameter calibration ($\sigma_i = \sigma/\sqrt{n}$)
- Constant-round (6 rounds) collaborative rejection sampling via Beaver triples
- MPC-Extended-GCD for distributed NTRU trapdoor generation
- Verifiable secret sharing with cheater detection
- Dynamic node management without public key modification
- Side-channel protected implementation
- Hardware acceleration integration (FPGA/TEE)
- Application to cross-chain bridge security
...
```
Claim 1 (System - Independent)
├── Claim 3 (Transform Mathematical Basis)
├── Claim 4 (Commit-Reveal Protocol)
├── Claim 7 (Hardware Acceleration)
├── Claim 9 (Output Format)
├── Claim 10 (NTRU Trapdoor)
├── Claim 15 (Cross-Chain Application)
│   └── Claim 16 (Bridge Architecture)
└── Claim 18 (Privacy Mechanism)

Claim 2 (Method - Independent)
├── Claim 11 (Communication Rounds)
├── Claim 12 (Local Sampling Details)
├── Claim 13 (Signature Aggregation)
├── Claim 14 (Verification Step)
└── Claim 17 (Quantum Security Basis)

Claims 1 or 2 (System or Method)
├── Claim 5 (Dynamic Node Management)
│   └── Claim 6 (Proactive Secret Sharing)
└── Claim 8 (Threshold Structure)
```
